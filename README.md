...
checkForPendingBookingConfirmation();
...
function initializeBookingForm() {
  ...
  bookingForm.addEventListener('submit', function(e) {
    ...
    // Store booking details in sessionStorage to show confirmation when user returns
    sessionStorage.setItem('pendingBookingConfirmation', JSON.stringify({
      customerName: customerName,
      bookingTotal: bookingTotal.toFixed(2),
      providerName: bookingState.provider.details.name,
      bookingDetails: bookingDetails
    }));
    
    // Send booking via WhatsApp
    if (bookingState.provider.details.support.whatsapp) {
...
function checkForPendingBookingConfirmation() {
  const pendingConfirmation = sessionStorage.getItem('pendingBookingConfirmation');
  if (pendingConfirmation) {
    const confirmationData = JSON.parse(pendingConfirmation);
    showProviderBookingConfirmation(confirmationData.customerName, confirmationData.bookingTotal);
